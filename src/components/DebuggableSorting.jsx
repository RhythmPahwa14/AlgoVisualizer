...
import DebuggingVisualizer from "./DebuggingVisualizer";
...
import { debuggableBubbleSort } from "../algorithms/debuggableBubbleSort";
import debuggableRunner from "../algorithms/debuggableRunner";
...
const ALGORITHM_MAPPINGS = {
  bubbleSort: { 
    ...
    debuggableFunction: debuggableBubbleSort,
    category: "comparison"
  },
  selectionSort: { 
    name: "Selection Sort", 
...
const DebuggableSorting = () => {
  ...
  const { state: debuggingState, actions: debuggingActions } = useDebugging();
  ...
  const handleSort = useCallback(async () => {
    ...
    try {
      ...
      if (state.isDebugging) {
        // Enable debugging in the runner
        debuggableRunner.enableDebugging();
        debuggableRunner.setDebuggingContext({ state: debuggingState, actions: debuggingActions });
        
        // Run with debuggable runner
        await debuggableRunner.runAlgorithm(
          algorithmHelpers.getName(state.algorithm),
          state.array,
          null // target (not used for sorting)
        );
      } else {
        // Run with regular algorithm function
  ...
  }, [state.isSorting, state.algorithm, state.array, state.delay, state.isDebugging, updateStats, debuggingState, debuggingActions]);
  ...
  const toggleDebugging = useCallback(() => updateState('isDebugging', !state.isDebugging), [state.isDebugging, updateState]);

  return (
    <div className="theme-container" data-aos="fade-up" data-aos-duration="1000">
      <h1 className="theme-title">Sorting Algorithms with Debugging</h1>
      <InputPanel
        dataType="array"
      ...
      {state.isDebugging && (
        <div className="theme-card">
          <div className="theme-card-header"><h3>Debugging Panel</h3></div>
          <DebuggingVisualizer algorithmName={algorithmName} initialArray={state.array} />
        </div>
      )}
